...
OpenVayra - Cities needs to preprocess OpenStreetMap (OSM) data to generate stop candidates. This requires `osmium-tool`, a C++ based command-line utility. Installing `osmium-tool` natively on developer machines (especially on Windows) can be inconsistent or complex.
...
5. **Semantic Boundary**: The `normalize-stop-candidates.mjs` script remains the OpenVayra - Cities semantic boundary, converting raw Osmium output into the internal candidate schema.
